...
class _ViewModifyMetalPricesViewState extends State<ViewModifyMetalPricesView> {
  ...
  Future<void> _openEditDialog(_MetalPriceItem item) async {
    ...
    var selectedMetalId = item.metalId;
    var selectedDate = item.priceDate;
      ...
      builder: (dialogContext) {
        return StatefulBuilder(
          builder: (dialogContext, setDialogState) {
            return AlertDialog(
              title: const Text('Update Metal Price'),
              content: SizedBox(
                    ...
                    child: Column(
                      ...
                      children: [
                        DropdownButtonFormField<String>(
                          initialValue: _metals.any((m) => m.id == selectedMetalId)
                              ? selectedMetalId
                              : null,
                          items: _metals
                              .map(
                  ...
                  onPressed: isUpdatingDialog
                    ...
                    try {
                      ...
                      await Supabase.instance.client.from('metal_prices').update({
                        ...
                        'price': parsedPrice,
                        'price_date': _formatDate(selectedDate),
                      }).eq('id', item.id);
                      ...
                      ScaffoldMessenger.of(dialogContext).showSnackBar(
                        const SnackBar(content: Text('Unable to update metal price.')),
                      );
                    }
                  },
